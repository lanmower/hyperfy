...
export class UI extends Node {
  ...
  resolveHit(hit) {
    if (hit?.point) {
      ...
      const hitPoint = new THREE.Vector3().copy(hit.point).applyMatrix4(inverseMatrix).multiplyScalar(1 / this._size).sub(this.pivotOffset)
      const x = hitPoint.x * this._res
      const y = -hitPoint.y * this._res
