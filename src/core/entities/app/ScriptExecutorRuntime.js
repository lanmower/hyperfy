...
export class ScriptExecutorRuntime {
  static parseScript(scriptCode, scripts) {
    try {
      ...
      const hyperfyError =
        parseErr instanceof HyperfyError
          ? parseErr
          : new HyperfyError('SCRIPT_ERROR', `Script parsing failed: ${parseErr.message}`, {
              originalError: parseErr.toString(),
            })
      throw hyperfyError
    }
  }
  ...
  static executeScript(evaluated, worldProxy, appProxy, fetchFn, safeProps, setTimeoutFn) {
    try {
      const appContext = evaluated.exec(worldProxy, appProxy, fetchFn, safeProps, setTimeoutFn)
      return appContext
    } catch (execErr) {
      const hyperfyError =
        execErr instanceof HyperfyError
          ? execErr
          : new HyperfyError('SCRIPT_ERROR', `Script execution failed: ${execErr.message}`, {
              originalError: execErr.toString(),
            })
      throw hyperfyError
    }
  }
    ...
    try {
      ...
      if (appContext.onLoad && typeof appContext.onLoad === 'function') {
        try {
          ...
          const hyperfyError =
            onLoadErr instanceof HyperfyError
              ? onLoadErr
              : new HyperfyError('SCRIPT_ERROR', `onLoad failed: ${onLoadErr.message}`, {
                  originalError: onLoadErr.toString(),
                })
      ...
      const hyperfyError =
        hookErr instanceof HyperfyError
          ? hookErr
          : new HyperfyError('SCRIPT_ERROR', `Failed to register script hooks: ${hookErr.message}`, {
              originalError: hookErr.toString(),
            })
  ...
  static unregisterHooks(app, listeners, recordError) {
    try {
      if (listeners.fixedUpdate && app) {
        app.off('fixedUpdate', listeners.fixedUpdate)
      }
      if (listeners.update && app) {
        app.off('update', listeners.update)
      }
      if (listeners.lateUpdate && app) {
        app.off('lateUpdate', listeners.lateUpdate)
      }
    } catch (cleanupErr) {
      const hyperfyError =
        ...
        try {
          ...
          const hyperfyError =
            err instanceof HyperfyError
              ? err
              : new HyperfyError('SCRIPT_ERROR', `onUnload failed: ${err.message}`, {
