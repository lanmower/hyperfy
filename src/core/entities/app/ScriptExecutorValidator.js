...
export class ScriptExecutorValidator {
  static validateScriptCode(scriptCode, app, blueprint) {
    ...
    if (typeof scriptCode === 'string') {
      ...
      const hyperfyError = new HyperfyError('TYPE_MISMATCH', `Invalid script format: ${typeof scriptCode}`, {
        phase: 'parseScript',
      })
      return { valid: false, error: hyperfyError }
    }
  ...
  static validateExecutedScript(evaluated) {
    ...
    if (!evaluated.exec || typeof evaluated.exec !== 'function') {
      const hyperfyError = new HyperfyError('INVALID_STATE', 'Evaluated script has no exec function', {
        phase: 'executeScript',
      })
      return { valid: false, error: hyperfyError }
    }
  ...
  static validateProxies(worldProxy, appProxy) {
    if (!worldProxy) {
      const hyperfyError = new HyperfyError('NULL_REFERENCE', 'World proxy not available', {
        phase: 'executeScript',
      })
      return { valid: false, error: hyperfyError }
    }
    ...
    if (!appProxy) {
      const hyperfyError = new HyperfyError('NULL_REFERENCE', 'App proxy not available', {
        phase: 'executeScript',
      })
      return { valid: false, error: hyperfyError }
    }
