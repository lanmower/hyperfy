import { clamp } from '../utils/helpers/misc.js'

let ids = 0
const arr1 = []

export class Curve {
  constructor() { this.keyframes = [] }

  deserialize(data) {
    if (!data) return this
    this.data = data
    this.keyframes = data.split('|').map(kData => new Keyframe().deserialize(kData))
    this.sort()
    return this
  }

  serialize() {
    return this.keyframes.map(keyframe => keyframe.serialize()).join('|')
  }

  add(opts) {
    const keyframe = new Keyframe().set(opts)
    const foundIndex = this.keyframes.findIndex(k => k.time === keyframe.time)
    if (foundIndex === -1) this.keyframes.push(keyframe)
    else this.keyframes[foundIndex] = keyframe
    this.sort()
    return this
  }
  ...
  getClosest(t) {
    ...
    let lo = -1, hi = this.keyframes.length
    while (hi - lo > 1) {
  ...
  evaluate(time) {
    ...
    if (time >= this.keyframes[this.keyframes.length - 1].time) return this.keyframes[this.keyframes.length - 1].value
    for (let i = 0; i < this.keyframes.length - 1; i++) {
      if (time >= this.keyframes[i].time && time <= this.keyframes[i + 1].time) {
        const t = (time - this.keyframes[i].time) / (this.keyframes[i + 1].time - this.keyframes[i].time)
        const p0 = this.keyframes[i].value, p1 = this.keyframes[i + 1].value
        const m0 = this.keyframes[i].outTangent * (this.keyframes[i + 1].time - this.keyframes[i].time)
        const m1 = this.keyframes[i + 1].inTangent * (this.keyframes[i + 1].time - this.keyframes[i].time)
  ...
  ogEvaluate(t) { return this.hermite(t, this.keyframes).y }
  ...
  hermite(t, keyframes) {
    ...
    var t2 = t * t, it = 1 - t, it2 = it * it, tt = 2 * t
    var h00 = (1 + tt) * it2, h10 = t * it2, h01 = t2 * (3 - tt), h11 = t2 * (t - 1)
    const x = h00 * keyframes[i0].time + h10 * keyframes[i0].outTangent * scale + h01 * keyframes[i1].time + h11 * keyframes[i1].inTangent * scale
    const y = h00 * keyframes[i0].value + h10 * keyframes[i0].outTangent * scale + h01 * keyframes[i1].value + h11 * keyframes[i1].inTangent * scale
    return { x, y }
  }
  ...
  move(keyframe, time, value, boundFirstLast) {
    ...
    if (keyIndex <= 0 || keyIndex >= this.keyframes.length - 1) {
      if (!boundFirstLast) keyframe.value = value
      return
    }
  ...
  clone() { return new Curve().deserialize(this.serialize()) }
}
...
export class Keyframe {
  ...
  serialize() {
    return [numToString(this.time), numToString(this.value), numToString(this.inTangent), numToString(this.outTangent)].join(',')
  }

  getHandles() { return { in: this.getInHandle(), out: this.getOutHandle() } }
  getInHandle() { return { x: this.time + this.inMagnitude, y: this.value + this.inMagnitude * this.inTangent } }
  getOutHandle() { return { x: this.time + this.outMagnitude, y: this.value + this.outMagnitude * this.outTangent } }
