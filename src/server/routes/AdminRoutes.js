...
export function registerAdminRoutes(fastify) {
  ...
  fastify.post('/api/admin/feature-flags/:flag/rollout', {
    preHandler: adminOnlyMiddleware,
  }, async (req, reply) => {
    return await APIMethodWrapper.wrapFastifyMethod(
      async () => {
        ...
        if (typeof percentage !== 'number') {
          return reply.code(400).send({ error: 'Percentage must be a number' })
        }

        logger.info('Set rollout', { flag, percentage })
        const result = setRollout(flag, percentage, 'admin')
      ...
      reply,
      { logger, defaultStatusCode: 500, defaultMessage: 'Failed to set rollout' }
    )
  })

  fastify.get('/api/admin/feature-flags/:flag/history', {
    preHandler: adminOnlyMiddleware,
  }, async (req, reply) => {
    return await APIMethodWrapper.wrapFastifyMethod(
      async () => {
        const { flag } = req.params
        logger.info('Get flag history', { flag })
        const history = getFlagHistory(flag)
      ...
      reply,
      { logger, defaultStatusCode: 500, defaultMessage: 'Failed to get flag history' }
    )
  })

  fastify.post('/api/admin/feature-flags/create', {
    preHandler: adminOnlyMiddleware,
  }, async (req, reply) => {
    return await APIMethodWrapper.wrapFastifyMethod(
      async () => {
        const { flag, description, enabled, rollout } = req.body

        if (!flag || !description) {
          return reply.code(400).send({ error: 'Flag name and description required' })
        }

        logger.info('Create feature flag', { flag })
        const result = createFeature(flag, description, enabled || false, rollout || 0, 'admin')
      ...
      reply,
      { logger, defaultStatusCode: 500, defaultMessage: 'Failed to create feature flag' }
    )
  })

  fastify.delete('/api/admin/feature-flags/:flag', {
    preHandler: adminOnlyMiddleware,
  }, async (req, reply) => {
    return await APIMethodWrapper.wrapFastifyMethod(
      async () => {
        const { flag } = req.params
        logger.info('Delete feature flag', { flag })
        const result = deleteFeature(flag, 'admin')
      ...
      reply,
      { logger, defaultStatusCode: 500, defaultMessage: 'Failed to delete feature flag' }
    )
  })

  fastify.get('/api/admin/feature-flags/:flag/variant/:userId', {
    preHandler: adminOnlyMiddleware,
  }, async (req, reply) => {
    return await APIMethodWrapper.wrapFastifyMethod(
      async () => {
        const { flag, userId } = req.params
        logger.info('Get user variant', { flag, userId })
        const variant = getUserVariant(flag, userId)
      ...
      reply,
      { logger, defaultStatusCode: 500, defaultMessage: 'Failed to get user variant' }
    )
  })
