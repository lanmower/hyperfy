...
// src/core/libs/csm/CSMCore.js
import * as three3 from "three";

// src/core/libs/csm/CSMShader.js
...
uniform vec2 CSM_cascades[${maxCascades}];
...
// src/core/libs/csm/CSMFrustum.js
import * as three2 from "three";
var inverseProjectionMatrix = new three2.Matrix4();
var CSMFrustum = class _CSMFrustum {
  constructor(data = {}) {
    this.vertices = {
      near: [new three2.Vector3(), new three2.Vector3(), new three2.Vector3(), new three2.Vector3()],
      far: [new three2.Vector3(), new three2.Vector3(), new three2.Vector3(), new three2.Vector3()]
    };
...
function practicalSplit(amount, near, far, lambda, target) {
  ...
  for (let i = 1; i < amount; i++) {
    target.push(three3.MathUtils.lerp(_uniformArray[i - 1], _logArray[i - 1], lambda));
  }
  target.push(1);
}
var _origin = new three3.Vector3(0, 0, 0);
var _lightOrientationMatrix = new three3.Matrix4();
var _lightOrientationMatrixInverse = new three3.Matrix4();
var _cameraToLightParentMatrix = new three3.Matrix4();
var _cameraToLightMatrix = new three3.Matrix4();
var _lightSpaceFrustum = new CSMFrustum();
var _center = new three3.Vector3();
var _bbox = new three3.Box3();
var _uniformArray = [];
var _logArray = [];
var CSM = class {
  constructor(data) {
    ...
    this.lightDirection = (_j = data.lightDirection) !== null && _j !== void 0 ? _j : new three3.Vector3(1, -1, 1).normalize();
    this.lightDirectionUp = (_k = data.lightDirectionUp) !== null && _k !== void 0 ? _k : three3.Object3D.DEFAULT_UP;
    this.lightIntensity = (_l = data.lightIntensity) !== null && _l !== void 0 ? _l : 1;
    this.lightColor = (_m = data.lightColor) !== null && _m !== void 0 ? _m : new three3.Color(16777215);
  ...
  createLights() {
    for (let i = 0; i < this.cascades; i++) {
      const light = new three3.DirectionalLight(this.lightColor, this.lightIntensity);
  ...
  injectInclude() {
    three3.ShaderChunk.lights_fragment_begin = CSMShader.lights_fragment_begin(this.cascades);
    three3.ShaderChunk.lights_pars_begin = CSMShader.lights_pars_begin(this.maxCascades);
  }
  ...
  getExtendedBreaks() {
    ...
    for (let i = 0; i < this.maxCascades; i++) {
      ...
      target.push(new three3.Vector2(prev, amount));
    }
...
// src/core/libs/csm/CSMHelper.js
import * as three4 from "three";
var CSMHelper = class extends three4.Group {
  constructor(csm) {
    super();
    ...
    const frustumGeometry = new three4.BufferGeometry();
    frustumGeometry.setIndex(new three4.BufferAttribute(indices, 1));
    frustumGeometry.setAttribute("position", new three4.BufferAttribute(positions, 3, false));
    const frustumLines = new three4.LineSegments(frustumGeometry, new three4.LineBasicMaterial());
  ...
  update() {
    ...
    while (cascadeLines.length < cascades) {
      const cascadeLine = new three4.Box3Helper(new three4.Box3(), new three4.Color(16777215));
      const planeMat = new three4.MeshBasicMaterial({
        transparent: true,
        opacity: 0.1,
        depthWrite: false,
        side: three4.DoubleSide
      });
      const cascadePlane = new three4.Mesh(new three4.PlaneGeometry(), planeMat);
      const shadowLineGroup = new three4.Group();
      const shadowLine = new three4.Box3Helper(new three4.Box3(), new three4.Color(16776960));
